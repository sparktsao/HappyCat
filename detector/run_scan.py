...
for f1 in Scanlist:
    ...
    a1 = feature_file_function(f1).split(",")
    a1 = np.asarray([a1],dtype="float32")
    t1 = timeit.default_timer()
    predicted = model.predict(a1)
    print "scanning...", f1
    t2 = timeit.default_timer()    
    tparse = t1-t0
    tpredi = t2-t1
    fwlog.write(sys.argv[0]+'\t'+f1+"\t"+sha1OfFile(f1)+"\t"+str(predicted[0][1])+"\t"+str(tparse)+"\t"+str(tpredi)+"\n")
    scanid=scanid+1
